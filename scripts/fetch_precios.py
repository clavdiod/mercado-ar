...
Fuente: BYMA Data (open.bymadata.com.ar) + CAFCI para FCI
"""

import json, time, logging, ssl, urllib.request, urllib.error
...
def fetch_json(url, extra_headers=None, retries=3, delay=5):
...
CAFCI_URL = "https://api.cafci.org.ar/fondo?estado=1&_=1"
CAFCI_HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0.0.0 Safari/537.36",
    "Accept": "application/json, text/plain, */*",
...
def parse_byma(data, cat):
    ...
    for item in (rows or []):
        ...
        cierre = clean(item.get("trade") or item.get("settlementPrice") or
                       item.get("closingPrice") or item.get("last") or item.get("price"))
...
def main():
    ...
    log.info("→ fci (CAFCI)...")
    raw   = fetch_json(CAFCI_URL, extra_headers=CAFCI_HEADERS)
    ...
    log.info(f"Resumen: {len(fechas)} fechas disponibles")

    if not todos:
        log.error("Sin datos de ninguna fuente")
        raise SystemExit(1)
