...
HEADERS = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124.0.0.0 Safari/537.36", "Accept": "application/json"}
...
def fetch_ticker(ticker):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15, context=SSL_CTX) as r:
            ...
            return {"ticker": ticker.replace(".BA",""), "nombre": name, "cierre": round(float(price),2), "variacion": var, "volumen": float(vol) if vol else None}
...
def main():
    ...
    with open(SUMMARY_FILE, "w", encoding="utf-8") as f:
        json.dump({"ultima_fecha": fechas[-1] if fechas else None, "fechas": fechas, "total_fechas": len(fechas), "actualizado_en": datetime.now().isoformat()+"Z"}, f, ensure_ascii=False, separators=(",",":"))
